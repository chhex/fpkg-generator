...
public class DaoMetaInfoSpringGenerator implements SchemaBasedVisitor<StringBuffer> {
    ...
    public DaoMetaInfoSpringGenerator(String nameSpace, String templatesRootDir) {
        this.groupDir = new STGroupDir(FilenameUtils.concat(templatesRootDir, "spring/meta"));
    ...
    public void visit(Pkg pkg) {
        var st = groupDir.getInstanceOf("daometa");
        var ns = String.format("%s.%s.%s", nameSpace, schema.toLowerCase(), NameConversion.toJavaName(pkg.getName()).toLowerCase());
        st.add("mmjavans", ns);
        st.add("mmxmlns", ns.replaceAll("\\.", "/"));
        outputBuffer.append(st.render(60));
    }

    @Override
    public void postvisit(Pkg pkg) {
        writeOutputFile("daoContext.xml",  NameConversion.toJavaName(pkg.getName()).toLowerCase(), outputBuffer.toString());
    }
